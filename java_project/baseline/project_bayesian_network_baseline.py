...
def train_model_and_result(alg_name, train_data_path, test_data_path, repo_name, open_filename,data_time):
    ...
    bayesian_model_path = "./rank_model/bayesian_network/" + repo_name + "/" + data_time + "/"
    ...
    # best_model=BayesianNetwork.load(filename=model_directory + repo_name+"_BayesianModel.bif", filetype="bif")
    print("===============结束训练模型+" + alg_name + "======================")

    print("===============开始利用模型计算测试集结果+" + alg_name + "======================")
    ...
    print("===============结束利用模型计算测试集结果+" + alg_name + "结束====结束结束结束结束=========")

    print("===============开始利用模型计算打开状态数据集结果并保存到数据库中+" + alg_name + "======================")
    open = pd.read_csv(open_filename)
    open_dataset = open.drop(columns=['pr_number'])
    open_dataset.dropna(inplace=True)
    open_dataset['priorities_number'] = open_dataset['priorities_number'].astype(int)
    predict_data = open_dataset.drop(columns='priorities_number', axis=1)
    print(set(predict_data.columns) - set(best_model.nodes()))
    ...
    for row_index in range(temp_data.__len__()):
        try:
            ...
            print("第", row_index, "行数据预测失败原因是: ", e)
            y_pred_temp = pd.DataFrame({"priorities_number": [0]})
        if y_open_pred is None:
            y_open_pred = y_pred_temp
        else:
            y_open_pred = y_open_pred.append(y_pred_temp, ignore_index=True)
    print(y_open_pred)
    ...
    for index in range(y_open_pred_series.__len__()):
        ...
        for key in open_pr_result.keys():
            if open_pr_result.get(key) == score and (sort_result.__len__() == 0 or sort_result.__contains__(key) is False):
                ...
                break
    print(sort_result)
    save_result_to_sql(data_time, repo_name, alg_name, sort_result)
    # 得到dict pr编号：预测的优先级
    print("===============结束利用模型计算打开状态数据集结果+" + alg_name + "结束====结束结束结束结束=========")
...
if __name__ == '__main__':
    ...
    # 测试模型性能的文件路径
    file_path = "../data_processing_engineering/bayesian_data/" + repo_name + "/" + data_time + "/"
    path_exists_or_create(file_path)
    origin_data_path = file_path + repo_name + "_bayes_rank_format_test_data_" + data_time + ".csv"
    bayesian_result_path = "./result/bayesian_network/" + repo_name + "/"
    path_exists_or_create(bayesian_result_path)
    result_path = bayesian_result_path + repo_name + "_" + alg_name + "_result_" + data_time + ".csv"
    # 训练模型的文件路径
    train_data_path = file_path + repo_name + "_bayes_rank_format_train_data_" + data_time + ".csv"
    test_data_path = file_path + repo_name + "_bayes_rank_format_test_data_" + data_time + ".csv"

    # 读取打开状态的PR列表，在这个文件中一块计算，然后保存到数据库中
